...
with sess.as_default():
    ...
    def CDF(ip):
        ...
        batch_size = tf.shape(F00)[0]
        #padding_tensor = tf.ones([batch_size, 1])
        t = K.concatenate(tensors=[tf.zeros([batch_size,5, 1]),F00,tf.ones([batch_size, 5,1])], axis=-1)*p3 + K.concatenate(tensors=[F00,tf.ones([batch_size, 5,1]),tf.ones([batch_size,5, 1])], axis=-1)*p1 + K.concatenate(tensors=[tf.zeros([batch_size, 5,1]),tf.zeros([batch_size, 5,1]),F00], axis=-1)*p2
        return t
    for i in range(T):
        Ft = keras.layers.Lambda(CDF)([Ft,p1,p2,p3])
    u_np = K.constant(np.random.uniform(0,1,batch_size))
    U = K.repeat_elements(u_np,rep=5*(2*T+1), axis=0)
    U = K.reshape(U, shape=(batch_size, 5, 2*T+1))#, input_shape=(batch_size*(2*T+1),))(U)
